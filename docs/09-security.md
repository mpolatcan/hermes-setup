...
- **The best sandbox is no shell at all.** Per Section 6.6, `terminal` and `code_execution` are disabled on `research`, `concierge`, `writer`, and `producer` — five of the agents have **zero** command-execution surface. Only `coder` (terminal + Python) and `ops` (terminal) can run host commands. Toolset pruning is the primary control now that there is no container.
