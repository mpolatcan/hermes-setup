...
> **Current stack (re-verified 2026-08-27):** primary model = **GPT-5.6 via Codex OAuth on all nine agents + crons** (`sol`: general/coder/researcher/writer; `terra`: assistant/finance/health; `luna`: marketing/producer), **DeepSeek V4 Flash is the only profile-level fallback** ([docs/04](04-models.md)); official upstream-clean Hermes Agent `v0.20.3` / `2026.8.16.2`; all 9 gateways 24/7; approvals off fleet-wide. Linear profile-plugin `0.8.14` remains active while `0.8.15` is the tested release candidate: create admission serializes the exact OPS+GAME workspace inventory, runs an immediate read-only retention classification at projected count 240–250, and denies projected count 250+ without dispatch. Its signed human `Done/Completed → In Progress` path creates one native AgentSession without another mention; duplicate, stale, self-authored, wrong-owner, wrong-team, drifted-revision, and open-session cases fail closed. Honcho uses authenticated `honcho.localhost` loopback config. Dated checklist entries below are historical build-log evidence, not current instructions.
...
- [x] **Role change — ops → marketing (2026-06-08).** The planned 7th agent was `ops`/Nilay (DevOps). With the watchdog owning the critical host-alert path deterministically, a shell-capable ops *agent* was redundant *and* doubled the arbitrary-code attack surface — so Nilay was repurposed to **marketing & community** (a real gap: nobody owned go-to-market; Ozan only writes craft prose). New slug `marketing`, MiniMax, web+TinyFish+cron, **no shell**. Net security win: at the time, **only `coder` had a shell** *(superseded 2026-06-08 — `general`/Derya later given an admin shell; see the Personal-tier log below + docs/09 §13.7a)*. Host monitoring stays the watchdog's job; build a real ops agent later only if you need NL host *diagnosis* (docs/10 open-Q).
