...
- **Install path.** Native Hermes via the official managed installer; Homebrew is retained only as an inert rollback package (see [Section 14](10-operations.md) for the exact upgrade story). All state lives under `~/.hermes/`.
- **Supervision is launchd's job.** The s6 tree only exists inside the Docker image. Native, each profile runs under its own `~/Library/LaunchAgents/ai.hermes.gateway-<profile>.plist`. The current process fleet was re-verified on official upstream-clean Hermes Agent `v0.20.3`; these LaunchAgents intentionally enter through the Keychain → 1Password SDK wrapper rather than being rewritten by an ad-hoc core patch.
...
- **Resource pressure has no hard cap.** Native gives no per-agent `--memory` ceiling. A runaway profile can swap the whole Mini. Mitigations: Hermes `max_turns` iteration budgets ([Section 13](09-security.md)) bound loops, the on-demand agents (`coder`, `writer`, `producer`) are light while idle (in practice all 9 gateways run 24/7 — an idle gateway is ~0.3–0.5 GB since inference is remote), and the **watchdog** ([Section 14.5](10-operations.md)) catches a down or crash-looping gateway within 5 minutes.
