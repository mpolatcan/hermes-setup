...
def copy_plugin(source: Path, destination: Path) -> bool:
    relative_files = (Path("plugin.yaml"), Path("__init__.py"), Path("scripts/codex_usage.py"))
    if destination.is_dir() and all(
        (destination / rel).exists() and (destination / rel).read_bytes() == (source / rel).read_bytes()
    ...
    try:
        ...
        shutil.copy2(source / "scripts" / "codex_usage.py", staging / "scripts" / "codex_usage.py")
...
def main() -> int:
    ...
    required = (source / "plugin.yaml", source / "__init__.py", source / "scripts" / "codex_usage.py")
