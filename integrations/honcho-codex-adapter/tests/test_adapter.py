...
class AdapterTests(unittest.TestCase):
    ...
    def test_driver_rejects_output_cap_without_operator_opt_in(self):
        request = ChatCompletionRequest.model_validate({
            "model": "honcho-deriver-luna",
            "messages": [{"role": "user", "content": "brief"}],
            "max_completion_tokens": 128,
        })
        with self.assertRaisesRegex(AdapterError, "operator opt-in"):
            HermesCodexDriver(allow_unbounded_output=False).complete(request)
            ...
            with self.subTest(expected_code=expected_code):
                ...
                with (
                    ...
                    with self.assertRaises(AdapterError) as caught:
                        HermesCodexDriver(allow_unbounded_output=True).complete(request)
