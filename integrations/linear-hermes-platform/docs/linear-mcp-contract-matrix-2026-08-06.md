...
- Transport: the initialization-era Streamable HTTP + JSON-RPC contract. Linear's live initialize response did not issue a session ID; the client nevertheless supports the negotiated-era optional session contract with bounded validation.
...
- The source worktree differs from loaded 0.8.3 runtime in project-null schema/forwarding, recursive null-policy narrowing, vendor execution authorization, mutation classification, 401 non-redispatch behavior, response-media validation, and session-ID validation. Documentation and tests are source-only. No promotion or restart was performed.
- Nine loopback health endpoints returned HTTP 200 with `status=ok`; nine public webhook endpoints returned 405 on GET, as expected.
- Nine launchd gateway labels were loaded; inbound/outbound SQLite integrity checks passed.
- Public DNS resolved consistently through 1.1.1.1 and 8.8.8.8.
- Fleet config audit found mutation allowlists broader than README intent on specialist profiles. This remains a rollout blocker; correcting config and restarting requires separate explicit approval.

## Rollback

No runtime mutation occurred in this audit phase. If a later approved promotion fails, use the immutable rollback path and digest printed by `scripts/deploy_plugin.py rollback`; never select a backup by recency.
