...
If no single active native session is available, the source channel receives the minimum fail-closed action: mention the display name returned for the installed Linear actor (for example, `@Doruk`) so Linear can create a fresh native AgentSession. The name is never profile-hard-coded. Closed sessions are never revived and the adapter never synthesizes `agentSessionCreate`.
...
Linear advertises `SUPPORTS_TRANSIENT_PROGRESS` separately from ordinary message editing. Gateway long-running notifications therefore bypass the append-only safety suppression only for this explicit capability. Every heartbeat carries the trusted per-turn delivery key plus typed `transient_progress` metadata and becomes a durable, idempotent, ephemeral `thought`; it never becomes a terminal `response`. Transient activities use a distinct outbox operation, so a permanently failed status cannot head-of-line block the later final response. The adapter deliberately does not expose ordinary `edit_message`, so generic tool-progress bubbles remain disabled on this append-only control plane. The gateway's default interval is three minutes unless the canonical Hermes runtime configuration changes it. A final `response`, `elicitation`, or `error` replaces the latest ephemeral status in Linear.
