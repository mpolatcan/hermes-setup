...
The tracked deployment allowlist is exactly `__init__.py`, `adapter.py`, `ledger.py`, `linear_client.py`, `oauth_store.py`, `mcp_client.py`, `outbound_policy.py`, `outbound_ledger.py`, `linear_tools.py`, `retention.py`, and `plugin.yaml`. Copy only those eleven files from `integrations/linear-hermes-platform/`; never copy tests, caches, credentials, OAuth stores, or SQLite state. The `0.8.15` release candidate requires all nine profiles to establish source/runtime hash parity, fresh process activation, healthy queues, SQLite integrity, typed OPS+GAME quota configuration, and profile-scoped real AgentSession read canaries. General additionally requires a real human reopen canary before `0.8.15` may be called the accepted fleet baseline. Earlier versioned acceptances are historical evidence, not proof of what is serving now. Exact entry sets, symlink status, directory/file modes, source/runtime hashes, process restart, and `/health` version must be established by a fresh live audit for each target.
