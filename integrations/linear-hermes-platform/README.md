...
The tracked deployment allowlist is exactly `__init__.py`, `adapter.py`, `ledger.py`, `linear_client.py`, `oauth_store.py`, `mcp_client.py`, `outbound_policy.py`, `outbound_ledger.py`, `linear_tools.py`, and `plugin.yaml`. Copy only those ten files from `integrations/linear-hermes-platform/`; never copy tests, caches, credentials, OAuth stores, or SQLite state. The current accepted fleet baseline is `0.8.13`: all nine profiles have source/runtime hash parity, fresh process activation, healthy queues, schema-v7 SQLite integrity, and profile-scoped real AgentSession acceptance for sanitized semantic commentary as an ephemeral non-terminal `thought` followed by a terminal cleanup `response`. Durable turn fences suppress late semantic/tool/heartbeat progress across restart, state eviction and replay until a genuinely different trusted turn opens. Earlier versioned acceptances are historical evidence, not proof of what is serving now. Exact entry sets, symlink status, directory/file modes, source/runtime hashes, process restart, and `/health` version must be established by a fresh live audit for each target.
