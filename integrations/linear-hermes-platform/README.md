...
Expected source result for this revision: `342/342 OK`. `/health` exposes the active inbound `data_event_types` allowlist, activation ambiguity counts, and closure counts so a rollout can verify the accepted event contract and drain state without inspecting source files.
