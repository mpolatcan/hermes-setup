...
if git diff --cached | grep -qE "ctx7sk-[a-zA-Z0-9-]{10,}|sk-[a-zA-Z0-9]{20,}|ghp_[a-zA-Z0-9]{20,}|xoxb-[a-zA-Z0-9-]{20,}|[0-9]{9,10}:AA[a-zA-Z0-9_-]{30,}"; then
  git reset -q
  /Users/mutlupolatcan/.hermes/scripts/hermes-send-keychain.sh general --to telegram "⚠️ config-snapshot: secret-shaped content staged — commit refused, fix the config" >/dev/null 2>&1 || true
  exit 1
fi
