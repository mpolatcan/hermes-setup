...
The source executable is `scripts/codex_usage_command.py`; the installer preserves
the deployed `scripts/codex_usage.py` filename for runtime compatibility. This
source migration does not apply the installer or restart gateways.
...
```bash
python3 -m unittest discover -s components/commands/codex-usage-command/tests -v
components/commands/codex-usage-command/scripts/codex_usage_command.py
```
