...
class FakeGoalManager:
    ...
    def __init__(self, session_id: str) -> None:
        ...
        self.set_calls = []
        self.state = (
            SimpleNamespace(status="active", created_at=123.0, turns_used=2)
            if self.existing
            else None
        )
    ...
    def set(self, goal: str, *, contract) -> object:
        ...
        self.state = SimpleNamespace(status="active", created_at=123.0, turns_used=0)
        return self.state

    def evaluate_after_turn(self, response: str, *, user_initiated: bool) -> dict:
        assert response
...
class NativeContinuationTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        self.adapter = LinearPlatformAdapter(
            PlatformConfig(enabled=True, extra={"database_path": path}), Platform.WEBHOOK
        )
        ...
        def admit(event: MessageEvent) -> bool:
            self.admitted.append(event)
            return True

        self.adapter.admit_internal_event = admit
        self.adapter.gateway_runner = SimpleNamespace(
            _profile_name_for_source=mock.Mock(return_value="general"),
    ...
    async def test_inflight_recovery_requests_followup_pass_for_new_enqueued_decision(self):
        self.adapter.admit_internal_event = mock.Mock(return_value=False)
