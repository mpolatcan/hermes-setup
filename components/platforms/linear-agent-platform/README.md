...
Non-streamed Linear AgentSession turns use Hermes core's post-turn delivery boundary. The adapter reads the AgentSession, issue, delegate, workflow state, and every blocker page again, combines that live evidence with core's immutable structured turn result and local Stop/closure fences, and persists exactly one of `success`, `continue`, `awaiting_input`, `approval`, `blocked`, or `stopped`. Ambiguous or incomplete evidence is `blocked`; only `success` may deliver the model response as a final Linear `response`.

For unfinished work, the adapter binds `hermes_cli.goals.GoalManager` to the same Hermes session. If no standing goal exists, it creates one from a bounded issue identifier/title and attaches a `GoalContract` requiring every live Linear acceptance checkbox and concrete PASS evidence. GoalManager alone owns evaluation, its turn budget, its judge, pause/exhaustion behavior, and the canonical continuation prompt. The adapter defines no second budget or prompt. A paused/exhausted/unavailable native goal becomes a visible Linear `error` and no turn is queued. `awaiting_input` and `approval` use `elicitation`; blocked/stopped work does not auto-continue.

A non-final turn summary is persisted through the existing outbox as an ephemeral `thought`, never delivered as a normal response. Before the canonical native prompt enters memory, SQLite records a deterministic decision and performs `pending → enqueued` CAS. The internal event reuses the Linear source and has a deterministic decision ID. Actual processing performs `enqueued → running`; completion performs `running → completed`. Startup reads only `pending`/`enqueued`, revalidates all live gates, reconstructs the prompt from GoalManager, and admits through core's multiplex-safe internal-event hook. A `running` row is never replayed and emits a deterministic visible error. Stop, cancellation, terminal issue state, awaiting-input/approval, delegate removal, closure, and newly observed blockers fence pre-start decisions under the session serialization boundary. Terminal decision rows follow normal retention; historical terminal failures do not degrade current health.

Schema v9 adds only `turn_decisions`: decision ID, Linear AgentSession UUID, issue ID, Hermes session ID, goal generation, ordinal, outcome, dispatch state, bounded error, and timestamps. It has no prompt, summary, description, or promptContext column, and uniqueness is `(agent_session_id, goal_generation, ordinal)`.
