...
The model-facing issue tool never accepts a raw workflow `state`. It exposes five semantic actions: `start`, `complete_child`, `cancel_child`, `enrich_plan`, and `mark_acceptance`. State actions cannot be bundled with another issue mutation. Guarded description actions accept only the issue ID, exact `expected_updated_at` revision, their constrained description payload, and—only for `mark_acceptance`—metadata-safe PASS evidence.
...
Runtime promotion, config mutation and `/restart` remain separate gates. Runtime extras are preserved inside the exact rollback tree rather than copied into the new twelve-file target.
...
Outbound tools are independently gated from the inbound webhook adapter. With `outbound_mcp.enabled: false`, no Linear model tools are registered. With `enabled: true` and `mutations_enabled: false`, only `linear_get_issue` and `linear_list_issues` are exposed. Mutation tools require both literal `mutations_enabled: true` and an explicit profile-local `allowed_mutation_tools` list. The current nine-profile rollout exposes `linear_save_issue` plus `linear_save_comment` to each exact profile identity so its delegate can perform guarded `mark_acceptance`; team, sensitive-mode and lifecycle policy remain profile-scoped and fail closed. A missing, empty, malformed, or unknown allowlist leaves the profile read-only.
...
```yaml
# Current profile-local shape (all nine profiles)
outbound_mcp:
...
Mutation authorization is capability-based rather than prompt-based. Registration preflights the complete tool-name set so a collision exposes no partial outbound surface, then applies the profile-local mutation allowlist. Derya may create issues, add same-team relations, delegate work, and write coordination comments without per-operation approval. Specialists report follow-up needs in their issue activity; Derya owns cross-agent task routing. The model-facing `linear_save_issue` schema does not expose raw `state`, and local policy rejects arbitrary state transitions even if a caller bypasses schema validation. It does preserve an explicit JSON `null` for `project`, matching the official `save_issue` contract and allowing a caller to clear project membership without conflating null with omission. No project lifecycle mutation is model-exposed: `linear_complete_project` is unknown to the registration allowlist and therefore fails closed. Human-owned main issues remain under Mutlu's terminal `Done` authority; only creator-owned child issues can use the narrow `complete_child` semantic action after exact creator/delegate/team/session and all-checked acceptance-evidence gates pass. Team, actor, organization, sensitive-data, authoritative relation, and idempotency gates remain fail-closed.
