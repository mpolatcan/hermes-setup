...
Top-level issues created by the installed app-user from a gateway-authorized Telegram DM use a narrower Direct path. The outbound tool records only task-local source identity, stable message/session pointers, operation key, policy result, and an issue fingerprint before vendor dispatch. Execution is authorized only after authoritative create read-back confirms the same creator, delegate, team, owner, top-level shape, and fingerprint. A model-provided Direct label is ignored. If the native session webhook wins the create/read-back race, its exact payload is durably parked and resumed on the same session after grant binding; dependency blockers still use the normal waiting ledger. Missing provenance, mismatch, ambiguous claim, stale reservation, or replay produces no additional execution. Human-created Planned Backlog→Todo and human-owned Done semantics remain unchanged.
...
## Native post-turn continuation

Non-streamed Linear AgentSession turns use only documented Hermes plugin/platform surfaces. This path is inert unless profile-local `native_goal_continuation_enabled: true` is set. The plugin records Hermes' structured `on_session_end` result, and the Linear adapter classifies it at its own final `send()` boundary before any response activity is admitted. The adapter reads the AgentSession, issue, delegate, workflow state, and every blocker page again, combines that live evidence with the immutable structured turn result and local Stop/closure fences, and persists exactly one of `success`, `continue`, `awaiting_input`, `approval`, `blocked`, or `stopped`. Ambiguous or incomplete evidence is `blocked`; only `success` may deliver the model response as a final Linear `response`. Continuations re-enter through the platform adapter's public `handle_message(event)` callback with the original public `SessionSource` and a strict pinned Hermes session ID. The feature gate is checked before goal or ledger mutation.

For unfinished work, the adapter binds `hermes_cli.goals.GoalManager` to the same Hermes session. If no standing goal exists, it creates one from a bounded issue identifier/title and attaches a `GoalContract` requiring every live Linear acceptance checkbox and concrete PASS evidence. GoalManager owns evaluation, the per-chunk turn budget, its judge, process-wait barrier, and the canonical continuation prompt. When the native budget pauses an otherwise actionable goal, the adapter performs a fresh Linear session/issue/delegate/state/blocker read-back, durably consumes one rollover allowance, and calls `GoalManager.resume(reset_budget=True)` on the same goal/session. The default cap is three rollovers and can be narrowed with profile-local `goal_max_budget_rollovers`; the durable count is not reset by normal turn-decision retention. Exhausting the cap or encountering any indeterminate lifecycle gate emits a visible error instead of looping. `awaiting_input` and `approval` use `elicitation`; blocked/stopped work does not auto-continue.

A non-final turn summary is persisted through the existing outbox as an ephemeral `thought`, never delivered as a normal response. Before the canonical native prompt enters memory, SQLite records a deterministic decision and performs `pending → enqueued` CAS. Returning from the inbound callback is scheduling, not admission: actual processing alone performs `enqueued → running` in `on_processing_start`, and completion performs `running → completed`. The public callback's existing session lease queues the continuation until the prior turn releases, preventing overlap. Callback exceptions atomically fence the decision with a visible error. Startup reads only `continue` decisions in `pending`/`enqueued`, revalidates all live gates, reconstructs the prompt from GoalManager, and replays it with the durable original `SessionSource`. A `running` row is never replayed and emits a deterministic visible error. Stop, cancellation, terminal issue state, awaiting-input/approval, delegate removal, closure, and newly observed blockers fence pre-start decisions under the session serialization boundary. Terminal activity and decision state are one SQLite transaction. A delayed successful response revalidates session, issue, delegate, workflow state, blockers, closure, and checked acceptance again immediately before Linear dispatch.

Schema v9 adds `turn_decisions`: decision ID, Linear AgentSession UUID, issue ID, Hermes session ID, goal generation, ordinal, outcome, dispatch state, bounded error, timestamps, and the payload-free public source-routing snapshot needed for restart. Schema v10 adds the payload-free `goal_budget_rollovers` counter and in-flight rollover marker. Neither table stores prompt, summary, description, promptContext, credential, or issue-body content.
...
/Users/mutlupolatcan/.hermes/runtime/hermes-gateway-sdk-bootstrap/venv/bin/python \
  components/platforms/linear-agent-platform/scripts/linear_mobile_pkce_once.py \
...
The tracked deployment allowlist is exactly `__init__.py`, `adapter.py`, `ledger.py`, `linear_client.py`, `oauth_store.py`, `mcp_client.py`, `outbound_policy.py`, `outbound_ledger.py`, `linear_tools.py`, `retention.py`, and `plugin.yaml`. Copy only those eleven files from `components/platforms/linear-agent-platform/`; never copy tests, caches, credentials, OAuth stores, or SQLite state. The `0.8.19` release candidate requires all nine profiles to establish source/runtime hash parity, fresh process activation, healthy queues, SQLite integrity, typed OPS+GAME quota configuration, and profile-scoped real AgentSession read canaries. General additionally requires a real human reopen canary before `0.8.19` may be called the accepted fleet baseline. Earlier versioned acceptances are historical evidence, not proof of what is serving now. Exact entry sets, symlink status, directory/file modes, source/runtime hashes, process restart, and `/health` version must be established by a fresh live audit for each target.
...
```bash
/Users/mutlupolatcan/.hermes/runtime/hermes-agent/venv/bin/python \
  components/platforms/linear-agent-platform/scripts/deploy_linear_agent_plugin.py deploy \
...
```bash
/Users/mutlupolatcan/.hermes/runtime/hermes-agent/venv/bin/python \
  components/platforms/linear-agent-platform/scripts/deploy_linear_agent_plugin.py rollback \
  --profiles-root /Users/mutlupolatcan/.hermes/profiles \
  --profile general \
...
Stop and delegate-removal events cancel a pending wait. Interrupted `resuming` rows return to `waiting` on adapter restart. `/health` reports waiting counts, oldest wait age, and the latest wait error; failed waits degrade health without causing a restart loop. The additive SQLite schema is versioned with `PRAGMA user_version=10`; version 3 adds issue/session linkage and durable closure evidence, version 4 adds a payload-minimized terminal-event fence, version 5 adds durable Planned activation waits and semantic activation claims, version 6 adds durable channel routing, version 7 adds durable progress-turn fencing across eviction and restart, version 8 adds Direct activation and closure bookkeeping, and version 9 adds native post-turn decisions without durable prompt or summary bodies. Back up `linear-bridge.sqlite3*` before first migration and retain the backup until live acceptance completes.
...
```bash
/Users/mutlupolatcan/.hermes/runtime/hermes-agent/venv/bin/python \
  components/platforms/linear-agent-platform/scripts/linear_retention_dry_run.py \
...
/Users/mutlupolatcan/.hermes/runtime/hermes-agent/venv/bin/python \
  -m unittest discover \
  -s components/platforms/linear-agent-platform/tests -v
```
