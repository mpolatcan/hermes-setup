# Linear official MCP contract matrix — 2026-08-06, live revalidated 2026-08-29
...
- Live inventory: **57 tools**. The 2026-08-29 probe added read-only `get_template` and `list_templates` to the 2026-08-27 55-name pin and added the optional `save_issue.template` property. The exact name set and draft-2020-12 URI are enforced for all 57. Exact root keys, property sets and requiredness are enforced for the five locally required vendor tools; type/nullability/format checks cover every locally forwarded field in those contracts. The local facade does not expose `template` and explicitly forwards `includeArchived=true` to preserve its established archived-inventory behavior.
...
| 22 | `get_template` | vendor-defined | Not model-exposed; drift validation only |
| 23 | `get_user` | `query` | Internal MCP↔GraphQL actor identity pin for every operation; not model-exposed |
| 24 | `get_workspace` | `—` | Internal workspace identity capability; not model-exposed |
| 25 | `list_agent_skills` | `—` | Not model-exposed; drift validation only |
| 26 | `list_comments` | `—` | Not model-exposed; drift validation only |
| 27 | `list_cycles` | `teamId` | Not model-exposed; drift validation only |
| 28 | `list_diffs` | `—` | Not model-exposed; drift validation only |
| 29 | `list_documents` | `—` | Not model-exposed; drift validation only |
| 30 | `list_issue_labels` | `—` | Not model-exposed; drift validation only |
| 31 | `list_issue_statuses` | `team` | Not model-exposed; drift validation only. Lifecycle guards use authoritative GraphQL lookups. |
| 32 | `list_issues` | `—` | linear_list_issues |
| 33 | `list_milestones` | `project` | Not model-exposed; drift validation only |
| 34 | `list_project_labels` | `—` | Not model-exposed; drift validation only |
| 35 | `list_projects` | `—` | Not model-exposed; drift validation only |
| 36 | `list_release_notes` | `—` | Not model-exposed; drift validation only |
| 37 | `list_release_pipelines` | `—` | Not model-exposed; drift validation only |
| 38 | `list_releases` | `—` | Not model-exposed; drift validation only |
| 39 | `list_teams` | `—` | Not model-exposed; drift validation only |
| 40 | `list_templates` | vendor-defined | Not model-exposed; drift validation only |
| 41 | `list_users` | `—` | Not model-exposed; drift validation only |
| 42 | `merge_diff` | `urlOrId` | Not model-exposed; drift validation only |
| 43 | `prepare_attachment_upload` | `issue, filename, contentType, size` | Not model-exposed; drift validation only |
| 44 | `resolve_diff_thread` | `threadId` | Not model-exposed; drift validation only |
| 45 | `save_comment` | `body` | linear_save_comment |
| 46 | `save_diff_comment` | `body` | Not model-exposed; drift validation only |
| 47 | `save_document` | `—` | Not model-exposed; drift validation only |
| 48 | `save_issue` | `—` | linear_save_issue |
| 49 | `save_milestone` | `project` | Not model-exposed; drift validation only |
| 50 | `save_project` | `—` | Not model-exposed; drift validation only |
| 51 | `save_release` | `—` | Not model-exposed; drift validation only |
| 52 | `save_release_note` | `—` | Not model-exposed; drift validation only |
| 53 | `save_status_update` | `type` | Not model-exposed; drift validation only |
| 54 | `search_documentation` | `query` | Not model-exposed; drift validation only |
| 55 | `share_issue` | vendor-defined | Not model-exposed; drift validation only |
| 56 | `submit_diff_review` | `urlOrId, decision` | Not model-exposed; drift validation only |
| 57 | `unshare_issue` | vendor-defined | Not model-exposed; drift validation only |
...
| Schema drift | All 57 tool names and schema dialects are pinned. Exact property sets, requiredness and `additionalProperties` are pinned for the five required vendor contracts; type/nullability/format checks cover every locally forwarded field. | `test_mcp_client.py` |
...
- The 2026-08-06 read-only snapshot SHA-256 `61fb5ed96b1883d08438ed5f8d23c26dac03d093e77bd617cfa55f9a8454e9b6` is historical and superseded by the 2026-08-29 live contract probe.
- Tool count: `57`; `get_template` and `list_templates` were added to the 2026-08-27 55-name pin. A fresh normalized full-schema hash will be recorded with runtime acceptance rather than reusing the historical digest.
...
Each approved promotion emitted an immutable per-profile rollback path and digest. Use only the recorded profile-specific artifact with `scripts/deploy_linear_agent_plugin.py rollback`; never select a backup by recency.
