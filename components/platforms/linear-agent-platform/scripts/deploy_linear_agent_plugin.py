...
REVIEWED_MANIFESTS: dict[str, dict[str, str]] = {
    ...
    "78085bedace2db262afe67953756ab65261f0490": {
        ...
        "outbound_policy.py": "20aee376ba0377df9091186ddd29ec7c895fd02934e255d06f254d2dabe0a091",
        "outbound_ledger.py": "aff0b2a6bd8f6fe933a28c8396cd7855b5929881cffbc2408ca89d74b1ca83e7",
        "linear_tools.py": "5bc059678ff294d4d90c17c3df46a2d895249b9de65d76ad4c2033cdada1a75e",
        "plugin.yaml": "80938d5975ef9e08d1a271a223db657c5037adbc7dd7f3c1a932bb2c0fe2f5f6",
    },
    "8db8f7ef17871343523f24b38e373dde459f098f": {
        "__init__.py": "0117a75173b9909b92137601e9725717c3d058c90b7551c710c94752207792a7",
        "adapter.py": "be092e8ac16452b59ed0047810362b6e6cfc940c68bc739f436db81658c6bcee",
        "ledger.py": "2bc25766cb61152e2c302a4a87b7ece075cc495e9331d9aa0d8ae1f8859df306",
        "linear_client.py": "8da34c13e1898f2416c28ba65315b6a3261ab7790a3df8535888c567b8073012",
        ...
        "outbound_policy.py": "20aee376ba0377df9091186ddd29ec7c895fd02934e255d06f254d2dabe0a091",
        "outbound_ledger.py": "aff0b2a6bd8f6fe933a28c8396cd7855b5929881cffbc2408ca89d74b1ca83e7",
        "linear_tools.py": "79d2b8a5484bbebc25650e04e014e79dd4819948d44bd9e2eadcefe48d7a0f0b",
        "plugin.yaml": "80938d5975ef9e08d1a271a223db657c5037adbc7dd7f3c1a932bb2c0fe2f5f6",
    },
    "609c4f181302b1c88bab99616c5988e655dea575": {
        "__init__.py": "0117a75173b9909b92137601e9725717c3d058c90b7551c710c94752207792a7",
        "adapter.py": "be092e8ac16452b59ed0047810362b6e6cfc940c68bc739f436db81658c6bcee",
        "ledger.py": "2bc25766cb61152e2c302a4a87b7ece075cc495e9331d9aa0d8ae1f8859df306",
        "linear_client.py": "8da34c13e1898f2416c28ba65315b6a3261ab7790a3df8535888c567b8073012",
    ...
    "5735bcde15a6987055baf79a3b7fd368ba0399a5": {
        "__init__.py": "0117a75173b9909b92137601e9725717c3d058c90b7551c710c94752207792a7",
        "adapter.py": "be092e8ac16452b59ed0047810362b6e6cfc940c68bc739f436db81658c6bcee",
        "ledger.py": "2bc25766cb61152e2c302a4a87b7ece075cc495e9331d9aa0d8ae1f8859df306",
        "linear_client.py": "8da34c13e1898f2416c28ba65315b6a3261ab7790a3df8535888c567b8073012",
        ...
        "outbound_policy.py": "20aee376ba0377df9091186ddd29ec7c895fd02934e255d06f254d2dabe0a091",
        "outbound_ledger.py": "aff0b2a6bd8f6fe933a28c8396cd7855b5929881cffbc2408ca89d74b1ca83e7",
        "linear_tools.py": "aaa0671a77443e786d58b687da7bc17fb20710167251558b659e04de23215b91",
        "plugin.yaml": "80938d5975ef9e08d1a271a223db657c5037adbc7dd7f3c1a932bb2c0fe2f5f6",
    },
    ...
    "6ee7f438f26328ea268cfb421ea3bb8a54a9020e": {
...
def _plugin_relative_for_commit(repo_root: Path, commit: str) -> Path:
    """Resolve the allowlisted plugin root without hard-coding legacy taxonomy."""
    listing = _run_git(repo_root, "ls-tree", "-r", "--name-only", commit)
    if not isinstance(listing, str):
        raise DeploymentError("Commit tree listing is invalid")
    files = {line for line in listing.splitlines() if line}
    if all((PLUGIN_RELATIVE / name).as_posix() in files for name in ALLOWLIST):
        return PLUGIN_RELATIVE

    candidate_roots = {
        Path(path).parent
        for path in files
        if Path(path).name == "plugin.yaml"
        and all((Path(path).parent / name).as_posix() in files for name in ALLOWLIST)
    }
    if len(candidate_roots) != 1:
        raise DeploymentError("Could not resolve a unique historical Linear plugin root")
...
def _manifest_from_commit(repo_root: Path, commit: str) -> dict[str, str]:
    ...
    """
    plugin_relative = _plugin_relative_for_commit(repo_root, commit)
    manifest: dict[str, str] = {}
    for name in ALLOWLIST:
        data = _run_git(
            repo_root,
            "show",
            f"{commit}:{(plugin_relative / name).as_posix()}",
            binary=True,
        )
