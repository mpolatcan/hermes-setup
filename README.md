...
**Status: fully deployed — 9/9 agents live** under launchd on the Mini. The fleet runs official upstream-clean Hermes Agent `v0.20.3` (`2026.8.16.2`); Linear `0.8.14` remains the active reviewed profile-local plugin while `0.8.15` is the tested release candidate for immediate critical-threshold retention dry runs and native human-reopen session activation. Honcho local authentication is config-only and keeps scoped JWTs in process memory. Linear alone disables direct interim display so completed Codex commentary reaches the official generic `on_interim_message` plugin hook; Telegram and other surfaces retain their normal display behavior.
