...
**Status: fully deployed — 9/9 agents live** under launchd on the Mini. The fleet runs official upstream-clean Hermes Agent `v0.20.2`; Linear is a reviewed profile-local plugin (`0.8.10` on the current general-profile canary, pending fleet parity), while Honcho local authentication is config-only and keeps scoped JWTs in process memory.
